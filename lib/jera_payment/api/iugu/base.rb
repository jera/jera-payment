  ...
  module Api
    class Iugu::Base
      ...
      def self.get(endpoint, query, account_access_token)
        HTTParty.get("#{JeraPayment.iugu_base_url}/#{endpoint}", headers: set_headers(account_access_token), query: query)
        parse_response(response)
      end

      def self.post(endpoint, body, account_access_token)
        HTTParty.post("#{JeraPayment.iugu_base_uri}/#{endpoint}", headers: set_headers(account_access_token), body: body.to_json)
        parse_response(response)
      end

      def self.put(endpoint, body, account_access_token)
        HTTParty.put("#{JeraPayment.iugu_base_uri}/#{endpoint}", headers: set_headers(account_access_token), body: body.to_json)
        parse_response(response)
      end

      def self.delete(endpoint, account_access_token)
        HTTParty.delete("#{JeraPayment.iugu_base_uri}/#{endpoint}", headers: set_headers(account_access_token))
        parse_response(response)
      end
